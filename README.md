...
- **Selective color grading** — lasso-select part of an image and grade just that region, independent of the rest
...
- **Music** — attach a trimmed audio clip to an image and export as a short MP4
...
- [Redis](https://redis.io/) — job queue ([BullMQ](https://docs.bullmq.io/)) and rate limiting ([rate-limiter-flexible](https://github.com/animir/node-rate-limiter-flexible))
- [Fabric.js](http://fabricjs.com/) — canvas engine powering crop, text, doodle, collage, and filters
...
2. Fill in `.env` with your own values — database credentials, R2 bucket/access keys, Resend API key, session secret, and (optionally) Google OAuth credentials. **Never commit your real `.env` file.**
...
2. In Imagine, go to **Settings → API Keys** and paste your token. It's encrypted at rest and only decrypted server-side at the moment of an API call.
